...
function ItemPage(props) {
  ...
  const submitActivate=()=>{
    ...
    Axios({
      method:"POST",
      url:`http://localhost:3000/activateItem/${item_id}`
    })
    .then((res)=>{
  ...
  const submitResponse = () => {
    ...
    Axios({
      url: `http://localhost:3000/confirmResponse/${messageId}`,
      method: "POST",
  ...
  const submitAnswer = () => {
    Axios({
      url:"http://localhost:3000/submitAnswer",
      method: "POST",
      data: {
        itemId: item_id,
            ...
            {itemimage.map((i) => {
              return (
                <div style={{ border: "2px solid black" }}>
                  <img
                    src={`https://lost-and-found-se.s3.ap-south-1.amazonaws.com/${i.img}`}
                    alt="item"
                  />
                </div>
              );
